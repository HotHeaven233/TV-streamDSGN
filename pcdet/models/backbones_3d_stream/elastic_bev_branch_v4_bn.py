...
def _group_count(channels, max_groups=32):
...
class SwitchableBatchNorm2d(nn.Module):
    """
    Per-width BatchNorm for the elastic path.

    Training keeps independent running statistics/affine parameters for each
    active width. Deployment fuses the selected BN into its materialized Conv,
    so there is no normalization kernel in the steady-state inference graph.
    """
    ...
    @torch.no_grad()
    def initialize_from_full(self, source_norm):
        for ratio in WIDTH_CHOICES:
            bn = self.bns[_ratio_key(ratio)]
            c = bn.num_features
            if getattr(source_norm, "weight", None) is not None:
                bn.weight.copy_(source_norm.weight[:c])
            else:
                bn.weight.fill_(1.0)
            if getattr(source_norm, "bias", None) is not None:
                bn.bias.copy_(source_norm.bias[:c])
            else:
                bn.bias.zero_()
            if getattr(source_norm, "running_mean", None) is not None:
                bn.running_mean.copy_(source_norm.running_mean[:c])
            else:
                bn.running_mean.zero_()
            if getattr(source_norm, "running_var", None) is not None:
                bn.running_var.copy_(source_norm.running_var[:c])
            else:
...
class ElasticFPN(nn.Module):
    def __init__(self, source_neck, res_max_channels):
        ...
        if source_neck.upconv_type != "fpn":
            raise RuntimeError("Elastic v4-BN currently requires upconv_type='fpn'")
        if not source_neck.with_upconv:
            raise RuntimeError("Elastic v4-BN requires with_upconv=True")
        if source_neck.start_level != 2:
            raise RuntimeError("Elastic v4-BN assumes feature_neck.start_level=2")
...
class PerWidthBEVProjection(nn.Module):
    """
    Independent lightweight 1x1 projection for each active RPN width.

    The heavy ResNet/FPN/stereo/RPN kernels remain shared and nested. Only this
    tiny semantic interface is width-specific so that 25/50/75/100% subnets do
    not fight over the same final projection columns.
    """

    def __init__(self, rpn_max_channels, bev_depth, output_channels):
        super().__init__()
        self.rpn_max_channels = int(rpn_max_channels)
        self.bev_depth = int(bev_depth)
        self.output_channels = int(output_channels)
        self.projections = nn.ModuleDict()

        for ratio in WIDTH_CHOICES:
            active_rpn = ratio_to_channels(self.rpn_max_channels, ratio)
            in_channels = active_rpn * self.bev_depth
            conv = nn.Conv2d(
                in_channels,
            ...
            with torch.no_grad():
                diagonal = min(in_channels, self.output_channels)
                for i in range(diagonal):
                    conv.weight[i, i, 0, 0] = 1.0
            self.projections[_ratio_key(ratio)] = conv

    def forward(self, x, ratio):
        conv = self.projections[_ratio_key(ratio)]
        if int(x.shape[1]) != int(conv.in_channels):
            raise RuntimeError(
                f"BEV projection width mismatch: ratio={ratio}, "
                f"got {x.shape[1]} channels, expected {conv.in_channels}"
            )
        return conv(x)
...
class ElasticBEVBranch(nn.Module):
    ...
    def __init__(self, full_backbone, output_bev_channels=96):
        super().__init__()
        if full_backbone.mono:
            raise RuntimeError("Elastic v4-BN requires stereo input")
        if full_backbone.drop_psv:
            raise RuntimeError("Elastic v4-BN requires PSV/cost-volume path")
        if full_backbone.cat_img_feature or full_backbone.cat_right_img_feature:
            raise RuntimeError(
                "Elastic v4-BN currently assumes cat_img_feature=False and "
                "cat_right_img_feature=False"
            )
        if len(full_backbone.hg_stereo) != 0:
            raise RuntimeError(
                "Elastic v4-BN currently targets the current K3 config with num_hg=0"
            )
        if len(full_backbone.rpn3d_hgs) != 1:
            raise RuntimeError(
                "Elastic v4-BN expects exactly one RPN3D hourglass in the K3 base"
            )
        ...
        self.bev_projection = PerWidthBEVProjection(
            self.rpn_max,
            self.bev_depth,
    ...
    def stage_stereo(self, batch_dict, state, full_backbone, ratio):
        ...
        # Match the original StreamDSGN exactly: dres1(cost0) + cost0,
        # with no extra ReLU after the residual addition.
        return x + residual
    ...
    def stage_rpn(
        ...
        bev = self.bev_projection(bev, ratio)
        if return_feature:
            return bev, valids, x
        return bev, valids
    ...
    def forward_with_features(
        ...
        """
        Training-only forward that also exposes native intermediate tensors for
        direct prefix-channel distillation against the frozen Full teacher.
        Inference keeps using forward()/forward_from_prefix() unchanged.
        """
        schedule = validate_schedule(schedule)

        state2 = self.stage_res2(prefix_cache, schedule[0])
        state3 = self.stage_res3(state2, schedule[1])
        state4 = self.stage_res4(state3, schedule[2])
        statef = self.stage_fpn(batch_dict, state4, schedule[3])
        stereo = self.stage_stereo(
            batch_dict, statef, full_backbone, schedule[4]
        )
        bev, valids, rpn_pool = self.stage_rpn(
            batch_dict,
            stereo,
            full_backbone,
            schedule[5],
        ...
        features = {
            "res2_left": state2["left_l2"],
            "res2_right": state2["right_l2"],
            "res3_left": state3["left_l3"],
            "res3_right": state3["right_l3"],
            "res4_left": state4["left_l4"],
            "res4_right": state4["right_l4"],
            "fpn_left": statef["left_stereo"],
            "fpn_right": statef["right_stereo"],
            "stereo": stereo,
            "rpn_pool": rpn_pool,
        }
