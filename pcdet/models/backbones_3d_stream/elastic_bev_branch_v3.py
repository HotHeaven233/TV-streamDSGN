...
class SwitchableGroupNorm2d(nn.Module):
    """
    Per-width GroupNorm without running statistics.

    The elastic path supports arbitrary cross-stage width transitions. BatchNorm
    running means/variances would otherwise mix multiple predecessor-width
    distributions for the same active width. GroupNorm removes this hidden
    train/test state while keeping a tiny per-width affine transform.
    """

    def __init__(self, max_channels, eps=1e-5):
        super().__init__()
        self.max_channels = int(max_channels)
        self.gns = nn.ModuleDict()
        for ratio in WIDTH_CHOICES:
            c = ratio_to_channels(self.max_channels, ratio)
            self.gns[_ratio_key(ratio)] = nn.GroupNorm(
                _group_count(c), c, eps=eps, affine=True
            )

    def forward(self, x, ratio):
        return self.gns[_ratio_key(ratio)](x)

    @torch.no_grad()
    def initialize_from_full(self, source_norm):
        # Preserve Full affine initialization when the source is BN/GN.
        if not hasattr(source_norm, "weight") or source_norm.weight is None:
            return
        for ratio in WIDTH_CHOICES:
            gn = self.gns[_ratio_key(ratio)]
            c = gn.num_channels
            gn.weight.copy_(source_norm.weight[:c])
            if getattr(source_norm, "bias", None) is not None:
                gn.bias.copy_(source_norm.bias[:c])


class SwitchableGroupNorm3d(nn.Module):
    def __init__(self, max_channels, eps=1e-5):
        super().__init__()
        self.max_channels = int(max_channels)
        self.gns = nn.ModuleDict()
...
class SlimConv2d(nn.Module):
    def __init__(
        ...
        self.norm = (
            SwitchableGroupNorm2d(self.max_out_channels) if norm else None
        )
...
class SlimConv3d(nn.Module):
    def __init__(
        ...
        self.norm = (
            SwitchableGroupNorm3d(self.max_out_channels) if norm else None
        )
        self.activation = bool(activation)
        nn.init.kaiming_normal_(self.weight, mode="fan_out", nonlinearity="relu")
...
class ElasticBasicBlock(nn.Module):
    ...
    def _init_shortcut_identity(self):
        ...
        for ratio in WIDTH_CHOICES:
            gn = self.shortcut.norm.gns[_ratio_key(ratio)]
            gn.weight.fill_(1.0)
            gn.bias.zero_()
