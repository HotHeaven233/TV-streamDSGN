...
class VoxelGeneratorWrapper:
    ...
    The generated voxels / coordinates / num_points follow the
    same NumPy interface expected by the original StreamDSGN code.
    """
    ...
    def __init__(
        ...
        if self.spconv_ver == 1:
            ...
            if tv is None:
                raise ImportError(
                    "spconv 2.x was detected, but "
                    "cumm.tensorview could not be imported"
                )

            self._voxel_generator = VoxelGenerator(
                vsize_xyz=list(vsize_xyz),
                coors_range_xyz=list(coors_range_xyz),
                num_point_features=int(num_point_features),
                max_num_points_per_voxel=int(max_num_points_per_voxel),
    ...
    def generate(self, points):
        ...
        if self.spconv_ver == 1:
            ...
            voxel_output = self._voxel_generator.generate(
                points
            )
            ...
            voxel_output = (
                self._voxel_generator.point_to_voxel(
            ...
            # .numpy() produces the NumPy representation expected
            # by the original StreamDSGN data pipeline.
...
class DataProcessor(object):
    def __init__(self, processor_configs, point_cloud_range, training):
        ...
        # Lazily constructed and reused.
        #
        # We cannot know num_point_features until the first point
        # array arrives, so the spconv generator is initialized on
        # the first call and then kept for later frames.
        self._voxel_generator = None
        self._stream_voxel_generator = None
    ...
    def transform_points_to_voxels(
        ...
        if data_dict is None:
            ...
            grid_size = (
                self.point_cloud_range[3:6]
                -
                self.point_cloud_range[0:3]
            ) / np.array(
                config.VOXEL_SIZE
            )
        ...
        (
            ...
            coordinates,
            num_points,
        ) = self._voxel_generator.generate(
            points
        )
    ...
    def transform_stream_points_to_voxels(
        self,
        data_dict=None,
        config=None,
        voxel_generator=None,
        ...
        if data_dict is None:
            ...
            return partial(
                self.transform_stream_points_to_voxels,
                config=config,
            )
        ...
        if self._stream_voxel_generator is None:

            self._stream_voxel_generator = (
                VoxelGeneratorWrapper(
                    vsize_xyz=
                        config.VOXEL_SIZE,
        ...
        (
            ...
            self._stream_voxel_generator.generate(
                points
            )
        )
