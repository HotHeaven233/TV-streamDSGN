...
START_ID="${5:-1}"
END_ID="${6:-84}"
LEVELS_JSON="${7:-outputs/elastic_bev/${ELASTIC_EXP_NAME:-elastic_bev_v4_bn_from_k3}/contention_calibration_v3/persistent_window/contention_levels.json}"
...
ROOT="outputs/elastic_bev/${EXP_NAME}/all84_contention_profile_v3/e${EPOCH}_n${N}"
TMP_DIR="${ROOT}/_tmp"
...
for ROW in "${LEVEL_ROWS[@]}"; do
  IFS=$'\t' read -r \
    LEVEL STRENGTH DURATION_MS START_DELAY_MS THREADS \
    <<< "${ROW}"
  ...
  echo
  echo "======================================================================"
  echo "${LEVEL}: persistent contention strength=${STRENGTH}"
  echo "duration=${DURATION_MS} ms; start-delay=${START_DELAY_MS} ms"
  echo "threads/block=${THREADS}"
  echo "profiles ${START_ID}..${END_ID}"
  echo "frames=${FRAMES}; warmup=${WARMUP}"
  echo "======================================================================"

  tail -n +2 "${SCHEDULE_TSV}" \
  | while IFS=$'\t' read -r \
      ID SCHEDULE TAG RES2 RES3 RES4 FPN STEREO RPN; do

    ID_NUM=$((10#${ID}))

    if [ "${ID_NUM}" -lt "${START_ID}" ] \
       || [ "${ID_NUM}" -gt "${END_ID}" ]; then
      continue
    fi
