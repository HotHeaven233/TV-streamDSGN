...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for bi, batch in enumerate(loader):
            ...
            # Convert AMP-scaled grads back to their true values before measuring.
            scaler.unscale_(optimizer)

            iter_id = used + 1
            do_grad_log = (
                args.grad_log_every > 0
                and (iter_id <= 3 or iter_id % args.grad_log_every == 0)
            )
            if do_grad_log:
                grad_groups = {
                ...
                for name, gs in grad_groups.items():
                    update_grad_summary(grad_summary, name, gs, args.grad_eps)
                    if gs['num_nonfinite'] > 0:
                        raise FloatingPointError(
                            f'Non-finite gradient in {name}: {gs}'
                        )
                print(
                    '[GRAD] '
                    + ' '.join(
                        f'{name}={gs["norm"]:.3e}'
                        f'(max={gs["max_abs"]:.2e},none={gs["num_none"]},'
                        f'bad={gs["num_nonfinite"]})'
                        for name, gs in grad_groups.items()
                    )
                )

            # clip_grad_norm_ returns the global norm BEFORE clipping.
            global_preclip = torch.nn.utils.clip_grad_norm_(params, args.grad_clip)
            if not torch.isfinite(global_preclip):
                raise FloatingPointError(
                    f'Non-finite global gradient norm: {global_preclip}'
                )
