...
def main():
    ...
    for level_name in ['L0', 'L1', 'L2', 'L3', 'L4']:
        ...
        root = Path(args.baseline_root) if level_name == 'L0' else Path(args.contention_root) / level_name
        ...
        for s in schedules:
            ...
            if not summary.exists():
                missing.append({'level': level_name, 'id': pid})
                continue

            t = json.loads(summary.read_text())
            q = qmap.get(pid, {})
            row = {
                'level': level_name,
                'busy_repeats': lev['busy_repeats'],
    ...
    ctrl_path = Path(args.output_controller_csv)
    ctrl_path.parent.mkdir(parents=True, exist_ok=True)
    if controller:
        with ctrl_path.open('w', newline='') as f:
            w = csv.DictWriter(f, fieldnames=list(controller[0].keys()))
            w.writeheader(); w.writerows(controller)
