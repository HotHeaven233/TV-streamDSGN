...
ANCHOR_SCHEDULES = tuple((r,) * 6 for r in WIDTH_CHOICES if r < 1.0)


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Elastic-v4-BN profile-space robust training for StreamDSGN K3. "
            "The Full K3 base stays frozen; only the elastic generator and "
            "its per-width BatchNorm/projection parameters are trained."
        )
    )
    ...
    parser.add_argument("--exp_name", type=str, default="elastic_bev_v4_bn")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional prior elastic checkpoint (v3 recommended). Matching shared "
            "convolution/projection tensors are copied. GroupNorm states do not "
            "match the new BatchNorm names and are intentionally skipped."
        ),
    )
    ...
    parser.add_argument(
        "--pre_bn_calibration_batches",
        type=int,
        default=336,
        help=(
            "Warm-start BatchNorm running statistics before gradient training. "
            "336 is about four balanced passes over the 83 trainable profiles."
        ),
    )
...
def verify_base_unchanged(model, snapshot):
    ...
    if changed:
        ...
        raise RuntimeError(
            "Frozen K3 base changed during Elastic-v4-BN training:\n" + detail
        )
...
class ProfileRobustSampler:
    ...
    def __init__(
        ...
        self.schedules = tuple(TRAIN_SCHEDULES)
...
def stage_distill_loss(
    ...
    for stage_name, keys in stage_names:
        # Leading 1.0 stages are executed by the frozen native Full modules in
        # v4. They have no elastic parameters and therefore need no distillation.
        if any(key not in student_features for key in keys):
            continue
        stage_terms = []
        for key in keys:
        ...
        active_count += 1
    if total is None or active_count == 0:
        ref = teacher_features["bev"]
        return ref.new_zeros(()), losses
    return total / active_count, losses
...
def iter_batchnorm_modules(module):
    for child in module.modules():
        if isinstance(child, (torch.nn.BatchNorm2d, torch.nn.BatchNorm3d)):
            yield child


@torch.no_grad()
def warm_start_bn_statistics(
    branch,
    base_model,
    train_loader,
    num_batches,
    ...
    bns = list(iter_batchnorm_modules(branch))
    if not bns:
        raise RuntimeError("Elastic-v4-BN branch contains no BatchNorm modules")
    ...
    for bn in bns:
        ...
        bn.momentum = None  # exact cumulative average during calibration
        bn.train()
    ...
    branch.train()
    logger.info(
        f"Warm-started BatchNorm statistics with {seen} hybrid-profile batches"
    )


def build_optimizer(branch, lr, weight_decay):
    decay = []
    no_decay = []
...
def save_checkpoint(
    ...
    torch.save(
        {
            "version": "elastic_v4_bn_profile_robust",
            "epoch": int(epoch),
            ...
            "num_runtime_schedules": len(MONOTONIC_SCHEDULES),
            "num_trainable_elastic_schedules": len(TRAIN_SCHEDULES),
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Elastic-v4-BN profile-space robust training")
    logger.info("Runtime prefix: native Full modules until the first width shrink")
    logger.info("Elastic stages: Res2, Res3, Res4, FPN, Stereo3D, RPN3D")
    logger.info("Normalization: per-width BatchNorm; final calibration + Conv/BN fusion")
    ...
    logger.info(f"runtime schedules: {len(MONOTONIC_SCHEDULES)}; trainable elastic schedules: {len(TRAIN_SCHEDULES)}")
    ...
    logger.info(f"Elastic-v4-BN trainable parameters: {trainable:,}")
    ...
    warm_start_bn_statistics(
        branch=branch,
        base_model=base_model,
        train_loader=train_loader,
        num_batches=args.pre_bn_calibration_batches,
    ...
    if args.resume is not None:
        ...
        logger.info(
            f"Resumed Elastic-v4-BN from {args.resume} at epoch={start_epoch}"
        )
        ...
        for iteration, batch_dict in enumerate(train_loader):
            ...
            for hist_index, (
                ...
                if hist_index in grad_slots:
                    ...
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        if tuple(hist_schedule) == FULL_SCHEDULE:
                            hist_bev = hist_teacher_bev
                        else:
                            hist_bev, _ = hybrid_forward(
                                branch,
                                frame,
                                hist_prefix,
                                base_model.backbone_3d,
                    ...
                    with torch.no_grad(), torch.cuda.amp.autocast(
                        ...
                        if tuple(hist_schedule) == FULL_SCHEDULE:
                            queue_bev = full_bev_from_prefix(
                                base_model,
                                frame,
                                hist_prefix,
                            )
                        else:
                            queue_bev, _ = hybrid_forward(
                                branch,
                                frame,
                                hist_prefix,
                                base_model.backbone_3d,
    ...
    with open(
        ...
        json.dump(
            {
                "version": "elastic_v4_bn_profile_robust",
                "runtime_prefix": "native Full modules until first width shrink",
                "elastic_stages": [
                    "ResNet-layer2",
                    "ResNet-layer3",
                    "ResNet-layer4",
                ...
                "normalization": (
                    "per-width BatchNorm; deployment fuses BN into materialized convolutions"
                ),
                "bev_projection": (
                    "independent 1x1 projection per output width"
                ),
                "width_choices": list(WIDTH_CHOICES),
                "num_runtime_schedules": len(
                    MONOTONIC_SCHEDULES
                ),
                "num_trainable_elastic_schedules": len(
                    TRAIN_SCHEDULES
                ),
                "native_full_schedule": list(FULL_SCHEDULE),
                "profile_sampler": {
    ...
    tb_log.close()
    logger.info("Elastic-v4-BN training completed; run final BN calibration before evaluation")
