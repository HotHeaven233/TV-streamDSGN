...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Timestamp-aligned capacity-1 streaming evaluation for Elastic-v4-BN: "
    ...
    parser.add_argument(
        "--mode",
        choices=("baseline_full", "elastic_fixed"),
        default="elastic_fixed",
    )
    parser.add_argument(
        "--fixed_schedule",
        type=str,
...
class ElasticRuntimeModel(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        # Fixed-width testing uses the actual v4 deployment graph.
        # All-Full bypasses the elastic branch completely. For a mixed profile,
        # leading 1.0 stages remain native and only the suffix after the first
        # shrink executes elastic fused-BN convolutions.
        if self.mode == "elastic_fixed":
            if tuple(self.fixed_schedule) == FULL_SCHEDULE:
                with torch.no_grad():
                    pred_dicts, ret_dicts = self.base_model(batch_dict)
                self.last_runtime_meta = {
                    "branch": "native_full",
...
def main():
    ...
    if "bn_calibration" not in checkpoint:
        print(
            "[WARN] checkpoint has no final BN calibration metadata. "
            "Run tools/calibrate_elastic_v4_bn.py before paper evaluation."
        )

    fused_counts = enable_fused_bn_static_cache(branch)
    print(f"[DEPLOY] fused BN static cache enabled: {fused_counts}")
    ...
    # Fair fixed-path warmup. v3 accidentally used --warmup only for dynamic
    # profiling, so the first streaming scene paid CUDA/kernel-cache startup.
    print(f"[warmup] {args.warmup} forwards before streaming evaluation")
    for _ in range(max(int(args.warmup), 0)):
        warm_batch = load_one(dataset, first_index)
        with torch.no_grad():
            runtime_model(warm_batch)
    torch.cuda.synchronize()
    if runtime_model.history_feature_queue is not None:
        runtime_model.history_feature_queue.clear()

    print("=" * 80)
    print("Elastic-v4-BN K3 streaming evaluation")
    print("fixed prefix      : native Full until first width shrink")
    print("elastic stages    : suffix after first shrink; BN fused into static Conv")
